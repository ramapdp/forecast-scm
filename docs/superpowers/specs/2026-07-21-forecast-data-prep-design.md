...
- **Calendar features**: day-of-week, day-of-month, month, is-weekend, Indonesian national holiday flag, Ramadan flag + days-into/until Ramadan, and flag + days-until/since proximity features for each of the 4 high-season events this business sees — Eid al-Fitr, Eid al-Adha, Indonesian Independence Day (Aug 17), and New Year's Day (Jan 1). Source: the `holidays` Python package's Indonesia calendar for Ramadan/Eid dates; Independence Day and New Year's Day are fixed Gregorian dates computed directly. Islamic holiday dates from the `holidays` package are spot-checked against known 2024/2025 dates before being trusted, given their business relevance to a Middle Eastern (Yaman/Kebuli) food business. Proximity features are left as raw days-until/since (capped at a 14-day window) rather than a single hand-picked "peak day" flag, so the model can learn each event's actual demand-peak offset from training data.
...
- **Region / lead-time features and the cumulative lead-time target** — `kawasan`/`hari_pengiriman` (delivery region and schedule) joined from `dataset/outlet_mapping.csv`, `lead_time_days` (days to the next delivery, variable per row from the transaction's day-of-week and region), and `target_lead_time_cumulative` (raw `Kuantitas` summed over the strictly-forward `lead_time_days`-day window) — the cumulative-demand-until-next-delivery target that is this project's core business goal. Full design and rationale in `docs/superpowers/specs/2026-08-08-lead-time-integration-design.md`.
